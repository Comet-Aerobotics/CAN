    ...
    def execute_callback(self, goal_handle: ServerGoalHandle):
        ...
        while rclpy.ok():
            ...
            if self.laser_tripped == True:
                # Bucket is full—disable actuator then check again
                if not vibrating:
                    # Disable actuator
                    self.actuator_voltage.publish(Float32(data=0.0))
                    vibrating = True
                    ...
                    # Check if delay should be over
                    vibe_elapsed = (now - vibration_start_time).nanoseconds / 1e9
                    if vibe_elapsed >= vibration_duration:
                        # Re-check laser
                        vibrating = False
                        # Small delay before re-checking (0.5s) using another timer
                        recheck_start = now
                        while (self.get_clock().now() - recheck_start).nanoseconds / 1e9 < 0.5:
                            if self.laser_tripped == False:
                                break
                        
                        # If still tripped after recheck, succeed
                        if self.laser_tripped == True:
                            self.stop_all()
                            goal_handle.succeed()
                            result.success = True
