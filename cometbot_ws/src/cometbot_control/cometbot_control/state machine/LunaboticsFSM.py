# cometbot_control/state_machine/LunaboticsFSM.py
import time
...
class LunaboticsFSM(Node):
    def __init__(self):
        ...
        # Logging helpers
        self._logged_once = set()               # messages already logged once
        self._throttle_last = {}                # key -> last log monotonic time

    # ---- logging helpers for Humble rclpy ----
    ...
    def log_throttle(self, period_sec, msg, key=None, level='info'):
        # key allows distinct throttles; default to message text
        k = key or msg
        now = time.monotonic()
        last = self._throttle_last.get(k, 0.0)
        if now - last >= period_sec:
            getattr(self.get_logger(), level)(msg)
            self._throttle_last[k] = now
    # ------------------------------------------
    ...
    def loop(self):
        ...
        if self.state == RoverState.STARTUP:
            ...
            self.log_once("Rover is idle — mission complete")
            twist.linear.x = 0.0
            twist.angular.z = 0.0
